...
from outlook_web.cluster.config import load_cluster_config
from outlook_web.cluster.storage import initialize_primary_schema, initialize_replica_schema
    ...
    def enforce_outlook_csrf_protection():
        ...
        try:
            ...
            if should_recover_same_origin_csrf_failure():
                ...
                g.csrf_same_origin_error_description = getattr(
                    error,
                    'description',
                    str(error),
                )
...
def decrypt_data(encrypted_data: str) -> str:
    ...
    try:
        ...
        try:
            app.logger.error(
                "Failed to decrypt sensitive data. SECRET_KEY may have changed or encrypted data may be corrupted."
            )
        except Exception:
            pass
...
def init_db():
    ...
    ''')
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS recipient_mail_links (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            account_id INTEGER NOT NULL,
            main_email_display TEXT NOT NULL,
            recipient_email_display TEXT NOT NULL,
            recipient_email_normalized TEXT NOT NULL,
            token_hash TEXT UNIQUE NOT NULL,
            token_encrypted TEXT NOT NULL DEFAULT '',
            expires_at TEXT,
            primary_access_count INTEGER NOT NULL DEFAULT 0,
            last_accessed_at TEXT,
            created_at TEXT NOT NULL,
            updated_at TEXT NOT NULL,
            FOREIGN KEY (account_id) REFERENCES accounts (id) ON DELETE CASCADE,
            UNIQUE (account_id, recipient_email_normalized)
        )
    ''')
    cursor.execute('''
        CREATE UNIQUE INDEX IF NOT EXISTS idx_recipient_mail_links_binding
        ON recipient_mail_links(account_id, recipient_email_normalized)
    ''')
    cursor.execute('''
        CREATE UNIQUE INDEX IF NOT EXISTS idx_recipient_mail_links_token_hash
        ON recipient_mail_links(token_hash)
    ''')
    cursor.execute('''
        CREATE INDEX IF NOT EXISTS idx_recipient_mail_links_expiry
        ON recipient_mail_links(expires_at, id)
    ''')
    cursor.execute('''
        CREATE INDEX IF NOT EXISTS idx_recipient_mail_links_search
    ...
    if 'account_id' not in public_mailbox_api_key_columns:
        cursor.execute(
            'ALTER TABLE public_mailbox_api_keys '
            'ADD COLUMN account_id INTEGER REFERENCES accounts(id) ON DELETE CASCADE'
        )
    cursor.execute('''
        CREATE INDEX IF NOT EXISTS idx_public_mailbox_api_keys_account
        ON public_mailbox_api_keys(account_id)
    ''')
...
MAILBOXES_MESSAGES_SCANNED_COUNT_SETTING = 'mailboxes_messages_scanned_count'
MAILBOXES_MESSAGES_SCANNED_COUNT_DEFAULT = 100
MAILBOXES_MESSAGES_SCANNED_COUNT_MIN = 1
MAILBOXES_MESSAGES_SCANNED_COUNT_MAX = 10000
MAILBOXES_MESSAGES_SCANNED_COUNT_VALIDATION_ERROR = '最多扫描邮件数必须是 1 到 10000 之间的整数'
...
def parse_mailboxes_messages_scanned_count(value: Any) -> tuple[Optional[int], str]:
    if value is None or isinstance(value, bool):
        return None, MAILBOXES_MESSAGES_SCANNED_COUNT_VALIDATION_ERROR
    ...
    if (
        parsed < MAILBOXES_MESSAGES_SCANNED_COUNT_MIN
        or parsed > MAILBOXES_MESSAGES_SCANNED_COUNT_MAX
    ):
        return None, MAILBOXES_MESSAGES_SCANNED_COUNT_VALIDATION_ERROR
...
def get_mailboxes_messages_scanned_count() -> int:
    parsed, error = parse_mailboxes_messages_scanned_count(
        get_setting(
            MAILBOXES_MESSAGES_SCANNED_COUNT_SETTING,
            str(MAILBOXES_MESSAGES_SCANNED_COUNT_DEFAULT),
        )
    )
    if error:
        return MAILBOXES_MESSAGES_SCANNED_COUNT_DEFAULT
    return parsed


PUBLIC_MAILBOX_API_KEY_AUTH_SETTING = 'public_mailbox_api_key_auth_enabled'
PUBLIC_MAILBOX_API_KEY_MASK_PREFIX = '\u2022' * 8
PUBLIC_MAILBOX_API_KEY_GENERATE_RETRY_LIMIT = 3
PUBLIC_MAILBOX_API_KEY_TOUCH_INTERVAL = timedelta(seconds=60)


class PublicMailboxApiKeyConflictError(RuntimeError):
    """Raised when a public mailbox API key digest already exists."""
...
def public_mailbox_api_key_utc_now() -> datetime:
    return datetime.now(timezone.utc)


def generate_public_mailbox_api_key() -> str:
    return f"pmk_{secrets.token_urlsafe(32)}"


def digest_public_mailbox_api_key(secret: str) -> str:
    return hashlib.sha256(str(secret).encode('utf-8')).hexdigest()


def _parse_utc_iso_z(value: Optional[Any]) -> Optional[datetime]:
    if value is None:
        return None
...
def _format_utc_iso_z(value: Optional[Any]) -> Optional[str]:
    parsed = _parse_utc_iso_z(value)
    if parsed is None:
        return None
    return parsed.isoformat().replace('+00:00', 'Z')


def _normalize_public_mailbox_api_key_secret(secret: Optional[str]) -> str:
    return str(secret or '').strip()


def _build_public_mailbox_api_key_record(row: sqlite3.Row) -> Dict[str, Any]:
    expires_at = _format_utc_iso_z(row['expires_at'])
    created_at = _format_utc_iso_z(row['created_at'])
    last_used_at = _format_utc_iso_z(row['last_used_at'])
    expires_dt = _parse_utc_iso_z(expires_at)
    now = datetime.now(timezone.utc)
    status = 'active'
    if expires_dt is not None:
        if expires_dt <= now:
            status = 'expired'
        elif expires_dt - now < timedelta(hours=72):
            status = 'expiring_soon'

    row_keys = set(row.keys())
    account_id = row['account_id'] if 'account_id' in row_keys else None
    account_email = row['account_email'] if 'account_email' in row_keys else None
    ...
    return {
        ...
        'masked_key': f"{PUBLIC_MAILBOX_API_KEY_MASK_PREFIX}{row['key_suffix'] or ''}",
...
def normalize_public_mailbox_api_key_account_id(account_id: Any) -> Optional[int]:
    if account_id is None or account_id == '':
        return None
    if isinstance(account_id, bool):
        raise ValueError('Public mailbox API key account does not exist')
    try:
        normalized_account_id = int(account_id)
    except (TypeError, ValueError) as exc:
        raise ValueError('Public mailbox API key account does not exist') from exc
...
def create_public_mailbox_api_key(
    name,
    remark='',
    expires_at=None,
    ...
    normalized_expires_at = _format_utc_iso_z(expires_at)
    normalized_account_id = normalize_public_mailbox_api_key_account_id(account_id)
    should_generate = secret is None or secret == ''
    normalized_secret = '' if should_generate else _normalize_public_mailbox_api_key_secret(secret)
    if not should_generate and not normalized_secret:
        raise ValueError('Public mailbox API key secret cannot be blank')
    max_attempts = PUBLIC_MAILBOX_API_KEY_GENERATE_RETRY_LIMIT if should_generate else 1
    db = get_db()

    for attempt in range(max_attempts):
        candidate_secret = generate_public_mailbox_api_key() if should_generate else normalized_secret
        created_at = utc_now_iso_z()
        key_suffix = candidate_secret[-4:] if len(candidate_secret) > 4 else ''
        encrypted_key = encrypt_data(candidate_secret)
        key_digest = digest_public_mailbox_api_key(candidate_secret)
        try:
            cursor = db.execute(
                '''
                INSERT INTO public_mailbox_api_keys (
                    name, remark, encrypted_key, key_digest, key_suffix,
            ...
            row = db.execute(
                '''
                SELECT k.*, a.email AS account_email
                FROM public_mailbox_api_keys k
                LEFT JOIN accounts a ON a.id = k.account_id
                WHERE k.id = ?
                ''',
                (cursor.lastrowid,),
            ).fetchone()
            return _build_public_mailbox_api_key_record(row)
        except sqlite3.IntegrityError as exc:
            db.rollback()
            if 'key_digest' not in str(exc).lower():
                raise
            if not should_generate:
                raise PublicMailboxApiKeyConflictError(
                    'Public mailbox API key already exists'
                ) from exc
            if attempt + 1 >= max_attempts:
                raise RuntimeError(
                    'Failed to generate a unique public mailbox API key'
                ) from exc

    raise RuntimeError('Failed to generate a unique public mailbox API key')


def list_public_mailbox_api_keys() -> List[Dict[str, Any]]:
    db = get_db()
    rows = db.execute(
        '''
        SELECT k.id, k.name, k.remark, k.key_suffix,
               k.expires_at, k.created_at, k.last_used_at, k.account_id,
    ...
    return [_build_public_mailbox_api_key_record(row) for row in rows]


def list_public_mailbox_api_key_binding_accounts() -> List[Dict[str, Any]]:
    rows = get_db().execute(
        'SELECT id, email FROM accounts ORDER BY LOWER(email), id'
    ).fetchall()
    return [
...
def get_public_mailbox_api_key_secret(key_id: int) -> Optional[str]:
    db = get_db()
    row = db.execute(
        'SELECT encrypted_key FROM public_mailbox_api_keys WHERE id = ?',
        (key_id,),
    ).fetchone()
    if row is None:
        return None
    try:
        return decrypt_data(row['encrypted_key'] or '')
    except Exception as exc:
        raise RuntimeError('Failed to decrypt data') from exc


def delete_public_mailbox_api_key(key_id: int) -> bool:
    db = get_db()
    cursor = db.execute(
        'DELETE FROM public_mailbox_api_keys WHERE id = ?',
        (key_id,),
    )
    db.commit()
...
def authenticate_public_mailbox_api_key(
    api_key: str,
    ...
    db = get_db()
    digest = digest_public_mailbox_api_key(candidate)
    row = db.execute(
        '''
        SELECT id, key_digest, expires_at, account_id
        FROM public_mailbox_api_keys
        WHERE key_digest = ?
        ''',
        (digest,),
    ).fetchone()
    if row is None or not secrets.compare_digest(row['key_digest'], digest):
        return False

    current = now or public_mailbox_api_key_utc_now()
    expires_at = _parse_utc_iso_z(row['expires_at'])
    if expires_at is not None and expires_at <= current:
        return False

    current_text = _format_utc_iso_z(current)
    cutoff_text = _format_utc_iso_z(
        current - PUBLIC_MAILBOX_API_KEY_TOUCH_INTERVAL
    )
    db.execute(
        '''
        ...
        ''',
        (current_text, row['id'], cutoff_text),
    )
    db.commit()
    return {
        'id': row['id'],
...
def is_public_mailbox_api_key_auth_enabled() -> bool:
    return get_setting(PUBLIC_MAILBOX_API_KEY_AUTH_SETTING) == 'true'


def set_public_mailbox_api_key_auth_enabled(enabled: bool) -> bool:
    return set_setting(
        PUBLIC_MAILBOX_API_KEY_AUTH_SETTING,
