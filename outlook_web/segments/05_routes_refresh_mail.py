...
def fetch_imap_account_detail_response(account: Dict[str, Any], folder: str,
                                       ...
                                       structured_error: bool = False) -> Dict[str, Any]:
    if structured_error:
...
def fetch_graph_detail_response(account: Dict[str, Any], folder: str,
                                ...
                                structured_error: bool = False) -> Optional[Dict[str, Any]]:
    if structured_error:
    ...
    if not detail:
        return None
    if detail.get('success') is False and detail.get('error'):
        return detail
...
def fetch_account_imap_emails_by_recipient(
    account: Dict[str, Any],
    folder: str,
    recipient: str,
    limit: int,
    ...
    folder_name = normalize_folder_name(folder)
    result = get_emails_imap_generic_by_recipient(
        account['email'],
        account.get('imap_password', ''),
        account.get('imap_host', ''),
        account.get('imap_port', 993),
...
def fetch_email_detail_for_account(account, message_id, method='graph', folder='inbox',
    ...
    if method == 'graph':
        ...
        if result and result.get('success') is False and structured_error:
            detail_error = result
        elif result:
            return result
    ...
    if result and result.get('success') is False and structured_error:
        ...
        if detail_error is None:
            ...
            if error_payload.get('status') == 504 or error_payload.get('code') == 'EMAIL_FETCH_TIMEOUT':
                detail_error = result
    elif result:
        return result
    if detail_error:
